...
const handleValueChange = (record: any, index: number, value: any) => {
  ...
  if (record.type === 'object' && record.childrenTable) {
    ...
    if (
      ...
      ['boolean', 'number', 'string'].includes(record.type)
    ) {
...
const handleAdd = async (index: number) => {
  ...
  if (index >= 0 && newTable[index]) {
    // 完全复制上一条数据
    newItem = structuredClone(newTable[index])
    ...
    // 复制第一条数据
    newItem = structuredClone(newTable[0])
  }
...
const handleDelete = async (index: number) => {
  ...
  try {
    ...
    console.error(
      `Failed to delete array item at ${itemToDelete.path || buildArrayPath(index)}:`,
      error,
    )
  }
}
